  ...
  <div class="col-lg-3">
              ...
              @foreach ($clients as $client)
                <option value="{{ $client->codicecf }}"
                  @if($client->codicecf==$cliente)
                      selected
                  @endif
                  >{{ $client->client->descrizion or 'cDeleted' }} [{{ $client->codicecf }}]</option>
              @endforeach
            </select>
          </div>
          <div>
    ...
    <div class="box box-default {{-- collapsed-box --}}">
      <div class="box-body">
        ...
        <a type="button" class="btn btn-default btn-block" href="{{ route('stFatt::idxAg') }}"><span>{{ trans('_menu.statsFatt')}} - {{trans('_menu.agent') }}</span></a>
      </div>
    </div>
